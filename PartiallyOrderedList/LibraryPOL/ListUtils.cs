namespace PartiallyOrderedList.LibraryPOL
{
    ...
    // Делегат для метода, выполняющего операции над элементом списка и не возвращающего результат
    public delegate T ActionDelegate<T>(T val);

    public delegate bool CompareDelegate<T>(T firstVal, T secondVal);
    ...
    public class ListUtils<T>
    {
        ...
        public static void ForEach(IList<T> list, ActionDelegate<T> actionDelegate)
        {
            try
            {
                if (list == null)
        ...
        public static void Sort<T>(IList<T> list, CompareDelegate<T> compareDelegate)
        {
            ...
            {
                concreteList.Sort((x, y) => Convert.ToInt32(compareDelegate(x, y)));
            }
            else
            {
                List<T> tempList = new List<T>(list);
                tempList.Sort((x, y) => Convert.ToInt32(compareDelegate(x, y)));
                for (int i = 0; i < list.Count; i++)
                {
                    list[i] = tempList[i];
                }
